...
These people had all apparently taken the quiz, and, incredibly enough, the sum total of their existences could be reduced to just two numbers: their position on the "social" axis, and their position on the "economic" axis.
...
The matrix factorization technique for recommender systems involves a similar trick, except the goal is to convince people to spend real money on snuggies, which is harder than convincing them to share something obnoxious on Facebook---most of us would do that for free anyway.

In other words, matrix factorization is the real deal---the stuff that separates authentic data scientists from  data alchemists, data phrenologists, data astrologers, and other kinds of data charlatans.
...
Sidenote: this style of reasoning is called "extracting actionable insights". It is the cornerstone of the data-scientific method, which sets modern data science apart from earlier, barbaric data pseudosciences.
