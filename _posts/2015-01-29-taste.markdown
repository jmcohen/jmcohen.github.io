...
The association between these two items could never be detected by content-based methods alone.
A Mozart collection has zero properties in common with a baby stroller.
...
These people had all apparently taken the quiz, and the sum total of their existences could, incredibly enough, be reduced to just two numbers: their position on the "social" axis, and their position on the "economic" axis.

This magic trick is called **dimensionality reduction**.

And what a feat!  Political platforms probably have hundreds of dimensions, like taxation, immigration, energy, national security,  climate change, and whether or not the candidate has ever driven to Canada with an Irish Setter tied to the roof of his car.
But the ideology quiz reduced every politician to just two: "social" and "economic."
...
The matrix factorization technique for recommender systems involves a similar trick, except the goal is to convince people to spend real money on snuggies, which is harder than the trivial task of convincing them to share something obnoxious on Facebook---something that most of us would do for free anyway.

In other words, matrix factorization is truly the stuff that sets the real data scientists apart from the data alchemists, data phrenologists, data astrologers, and other flavors of data charlatan.
...
(Sidenote: this style of reasoning, called "drawing actionable insights," is the cornerstone of the data scientific method, which distinguishes modern data science from earlier, barbaric data pseudosciences.)
...
To pull up the taste breakdown for any food, just start typing the name of the food into the search bar, and then click on one of the recipes that pop will up in the autocomplete box.

Clicking on any of the example recipes that show up will pull up *their* taste breakdowns.
